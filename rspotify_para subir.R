...
uruguay$uno <- 1
canciones_por_artista_uru  <- aggregate(uno~artista, data=uruguay , sum)

artistas.uru <- merge(artistas.uru, canciones_por_artista_uru , by=c("artista"), all.x=TRUE)

ggg <-ggplot (artistas.uru, aes(x=genero_rec, y=uno, fill=genero_rec)) + 
  geom_bar(stat="identity", position="stack") + xlab("Género musical") + ylab("Cantidad de canciones en top 50") +
  ggtitle("Uruguay ") + theme(legend.position="", 
                              axis.text=element_text(size=12), 
                              axis.title=element_text(size=12)) +
  scale_y_continuous(breaks=c(2,4,6,8,10,12), limits=c(0,12))
ggg

#pondero y grafico argentina 

argentina$uno <- 1
canciones_por_artista_arg  <- aggregate(uno~artista, data=argentina , sum)

artistas.arg <- merge(artistas.arg, canciones_por_artista_arg , by=c("artista"), all.x=TRUE)

ggg2 <-ggplot (artistas.arg, aes(x=genero_rec, y=uno, fill=genero_rec)) + 
  geom_bar(stat="identity", position="stack") + xlab("Género musical") + ylab("Cantidad de canciones en top 50") +
  ggtitle("Argentina ") + theme(legend.position="", 
                                axis.text=element_text(size=12), 
                                axis.title=element_text(size=12)) +
  scale_y_continuous(breaks=c(2,4,6,8,10,12), limits=c(0,12))
ggg2
...
#pondero y grafico venezuela

venezuela$uno <- 1
canciones_por_artista_vene  <- aggregate(uno~artista, data=venezuela , sum)

artistas.vene <- merge(artistas.vene, canciones_por_artista_vene , by=c("artista"), all.x=TRUE)

ggg3 <-ggplot (artistas.vene, aes(x=genero_rec, y=uno, fill=genero_rec)) + 
  geom_bar(stat="identity", position="stack") + xlab("Género musical") + ylab("Cantidad de canciones en top 50") +
  ggtitle("Venezuela ") + theme(legend.position="", 
                                axis.text=element_text(size=12), 
                                axis.title=element_text(size=12)) +
  scale_y_continuous(breaks=c(2,4,6,8,10,12), limits=c(0,12))
ggg3

png("canciones_arg uru vene.png", height=800, width=800)
multiplot(ggg, ggg2, ggg3, cols=2)
dev.off()
